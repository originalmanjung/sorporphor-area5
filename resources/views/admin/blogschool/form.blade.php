...
<div class="container-fluid px-4">
    <h1 class="mt-4">Blog School</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item active">จัดการกิจกรรมโรงเรียนในสังกัด</li>
    </ol>

    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div><i class="fas fa-table me-1"></i>สร้าง</div>
            <a href="{{ route('app.blogSchools.index') }}" type="button" class="btn btn-danger"><i class="fas fa-arrow-circle-left"></i> ย้อนกลับ</a>
        </div>
        <div class="card-body">
            <form class="row g-3" id="blogschoolFrom" user="form" method="POST" action="{{ isset($blogSchool) ? route('app.blogSchools.update',$blogSchool->id) : route('app.blogSchools.store') }}" enctype="multipart/form-data">
                @csrf
                @if (isset($blogSchool))
                    @method('PUT')
                @endif
                <div class="col-md-6">
                    <div class="col mb-3 form-group">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ $blogSchool->title ?? old('title') }}" autocomplete="title" autofocus>
                        @error('title')
                    ...
                    <div class="col mb-3 form-group">
                        ...
                        <textarea class="form-control" id="exampleFormControlTextarea1" name="description" rows="5">{{ $blogSchool->description ?? old('description') }}</textarea>
                        @error('description')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="row g-2">
                        <div class="col-md mb-3">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="status" name="status" @if(isset($blogSchool)) {{ $blogSchool->status == true ? 'checked' : '' }} @endif>
                        ...
                        <div class="mb-3">
                            ...
                            @foreach ($blogSchool->blogSchoolPhotos as $key=>$photos )
                                    <div class="col-xl-3 col-lg-4 col-md-6 mb-4 image-area">
                                        <a class="thumbnail fancybox" rel="ligthbox" href="{{ asset('storage/blogschools_photos/'.$photos->filename) }}">
                                        ...
                                        <form id="delete-form-{{ $photos->id }}" action="{{ route('app.blogSchoolPhotos.destroy',$photos->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
            ...
            @endisset
            <div class="col-12">
                <button class="btn btn-primary" onclick="showLoading(@isset($blogSchool) 'กำลังอัฟเดทข้อมูล...' @else 'กำลังเพิ่มข้อมูล...' @endisset,document.getElementById('blogschoolFrom').id);">
                    @isset($blogSchool)
...
@push('js')
    ...
    <script>
        var upload = new FileUploadWithPreview("myUniqueUploadId", {
            ...
            @isset($blogSchool->avatar)
            presetFiles: [
                "{{ asset('storage/user_avatars') }}/{{ $blogSchool->avatar }}",
